...
export function Dashboard() {
  ...
  const currentFilters = (f: FilterForm): AnalyticsFilters => ({
    year: f.year ?? null,
    month: f.month ?? null,
  ...
  const loadAnalytics = (f: FilterForm) => {
    setLoading(true);
    setError(null);
    const filt = currentFilters(f);
  ...
  useEffect(() => {
    ArticlesApi.list({ page: 1 })
      .then((page) => setArticleCount(page.count))
  ...
  const applyFilters = () => loadAnalytics(filters);
  const clearFilters = () => {
    ...
    <div className="dashboard">
      <header className="dashboard__head">
        <div>
          ...
          <p>Interactive analytics for all Wordetica articles.</p>
        </div>
      ...
      <div className="wo-grid wo-grid--3 stats">
        <article className="stat">
          <span className="stat__label">Articles in DB</span>
          <span className="stat__value">{articleCount ?? "-"}</span>
        ...
        <article className="stat">
          <span className="stat__label">Quick action</span>
          <span className="stat__value">New article</span>
        ...
        <article className="stat">
          ...
          <span className="stat__value">{user?.email}</span>
          {isAdmin && <span className="wo-tag">Admin</span>}
        </article>
      </div>
      ...
      <section className="charts-grid">
        <article className="chart-card">
          <header>
            ...
            <span className="hint">Click a bar</span>
          </header>
          <div className="chart">
            <ChartCanvas
              type="bar"
        ...
        <article className="chart-card">
          <header>
            ...
            <span className="hint">Click a slice</span>
          </header>
          <div className="chart">
            <ChartCanvas
              type="doughnut"
        ...
        <article className="chart-card">
          <header>
            ...
            <span className="hint">Click a point</span>
          </header>
          <div className="chart">
            <ChartCanvas
              type="line"
      ...
      {drilldownTitle && (
        <section className="chart-card drilldown">
          <header>
            <h2>
      ...
      {unused && (
        <section className="chart-card unused">
          <header>
